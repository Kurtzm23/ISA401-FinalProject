...
pacman::p_load(tidyverse, rvest, lubridate, httr, xml2, openxlsx
               )
...
# Trying something, might be stupid

# My shame-correcting URLs ------------------------------------------------
...
# adding the genre and stuff ----------------------------------------------
rotten_adds = tibble()
for (i in 1:length(rotten_title_s3)) {
  rotten_prof_stem = 'https://www.rottentomatoes.com/m/'
  rotten_prof_mov = tolower(rotten_title_s3[i])
...
# My shame--correting URLs ------------------------------------------------

  
  rotten_profile = tibble(id = i,
                          Title = rotten_title_s3[i], 
                          Genre = rotprof_s3, 
                          Duration = rotdur_s3)
  rotten_adds = rbind(rotten_adds, rotten_profile)
}
# So it works, but at what cost, there's some missing
# So now I'm thinking I should undo the changes I made and 
# Try to do an if statement like if the Adds matches the 
# original table append the extra info and if not make it 
# null


# * Left Join -------------------------------------------------------------

both_df = left_join(x = rotten_adds %>% select(-Title), y = rotten_tbl, by = 'id') %>% 
  select(-id)

hr_cleaner = function(x){
  hr = str_extract(x, pattern = '[:digit:]{1}')
  return(as.numeric(hr))
}

both_df$hr = hr_cleaner(both_df$Duration) * 60
both_df$min = min_ext(both_df$Duration)

both_df$Duration = both_df$hr + both_df$min


min_ext = function(x){
  min = str_extract(x, pattern = '[:space:][:digit:]{2,}') %>% str_remove(pattern = ' ')
  return(as.numeric(min)) %>% na.omit()
}
